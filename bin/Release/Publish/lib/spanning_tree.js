 ...
 */
function build_tree(dungeonOBJ, do_extra_points = true) {
    ...
    if (do_extra_points) {
        try {
            const room_spacing = Math.round(
                (21 - get_dungeon_configuration("room_density", dungeonOBJ).size )
                    +
                ((100 - get_dungeon_configuration("corridor_layout", dungeonOBJ).pct) / 10)
            );
            ...
            const points_to_keep = Math.max(Math.round(junction_points.length / 3), 1);
    ...
    var g = {};
    try {
        g = new jsgraphs.WeightedGraph(points.length);
    ...
    var mst;
    try {
        kruskal = new jsgraphs.KruskalMST(g);
        mst = kruskal.mst;
