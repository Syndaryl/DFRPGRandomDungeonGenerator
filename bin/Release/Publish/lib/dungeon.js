...
function init_form() {
    ...
    Global_Dungeon.cell_size =  parseInt($('Resolution').getValue()) * 2;
 ...
 * @param {string} get_item_key - Key to getItem in localStorage. Default: 'last_saved_query'
 */
function load_dungeon_configuration(get_item_key) {
    if (get_item_key === undefined) get_item_key = 'last_saved_query';

    if (supports_local_storage()) {
        var dungeon_options_to_load = JSON.parse(localStorage.getItem(get_item_key));
        $H(dungeon_options).keys().each(function(key_primus) {
            $(key_primus).setValue(dungeon_options_to_load[key_primus]);
        });
        $('dungeon_name').setValue(dungeon_options_to_load['seed']);
        $('Resolution').setValue(dungeon_options_to_load['resolution'] || 25);
        update_self_link();
    }
}
/**
...
function save_dungeon_configuration(set_item_key) {
    if (set_item_key === undefined) set_item_key = 'last_saved_query';
    var dungeon_options_to_save = {};
    $H(dungeon_options).keys().each(function(key_primus) {
        dungeon_options_to_save[key_primus] = $(key_primus).getValue();
    });
    dungeon_options_to_save['seed'] = $('dungeon_name').getValue();
    dungeon_options_to_save['resolution'] = parseInt($('Resolution').getValue()) || 25;
	if (supports_local_storage())
...
function html_dungeon_details(dungeonOBJ){
    ...
    //    dungeon_debug.classList.remove('hidden');
    //} else {
    //    tree.classList.add('hidden');
    //    dungeon_debug.classList.add('hidden');
    //}
}
...
function html_room_details(dungeonOBJ) {
    ...
    for (var roomItr = 1; roomItr <= dungeonOBJ.n_rooms; roomItr++) {
        ...
        $(anchor).setAttribute('name', 'room_' + room['id']);
        $(anchor).setAttribute('id', 'room_' + room['id']);
...
function html_room_doors(element, room) {
    ...
    for(var direction = 1; direction <= 4; direction++){
        var dir = cardinal_dirs_enum_invert[direction];
...
function create_dungeon() {
    ...
    if (dungeonOBJ === null) return null;
    dungeonOBJ = emplace_rooms(dungeonOBJ);
    dungeonOBJ = order_rooms(dungeonOBJ);
    dungeonOBJ = label_rooms(dungeonOBJ);
    var room_layout = dungeonOBJ.room_layout;
    if (room_layout === 'experimental' || room_layout === 'directed_graph') {
...
function init_dungeon() {
    ...
    dungeonOBJ.cer = parseInt($('cer').getValue());
    dungeonOBJ.resolution = parseInt( $('Resolution').getValue());
    var dungeon_size = get_dungeon_configuration('dungeon_size', dungeonOBJ),
        ...
        cellsize = dungeonOBJ.resolution;

    if (!dungeonOBJ.map_style) return null;
    //cellsize =  b.cell();
    //b = b.cell;
    ...
    if (layout.mask !== undefined && layout.mask !== null) {
...
function emplace_rooms(dungeonOBJ) {
    ...
    switch (dungeonOBJ.room_layout) {
        ...
        case 'directed_graph':
            if (get_dungeon_configuration('room_density', dungeonOBJ).size < 3)
                return dense_rooms_digraph(dungeonOBJ);
            else
                return tight_rooms(dungeonOBJ, room_spacing);
        default:
            return tight_rooms(dungeonOBJ, room_spacing);
 ...
 */
function dense_rooms_digraph(dungeonOBJ) {
    var itr_a;
    var index = [];
    // build the index
    for (var size in dungeon_options.room_size) {
        if (Object.prototype.hasOwnProperty.call(dungeon_options.room_size, x)) {
            index.push({ 'key': size, 'index': dungeon_options.room_size[size]['index'] });
        }
    }
    ...
    var roomMax = index.findIndex(function (i) { return i['key'] === dungeonOBJ.room_size; });
    for (var roomSizeCurrent = roomMax; roomSizeCurrent >= 0; roomSizeCurrent--) { // for each room size
        for (itr_a = 0; itr_a < dungeonOBJ.n_i; itr_a++) {
            var x = itr_a * 2 + 1,
                d;
            for (d = 0; d < dungeonOBJ.n_j; d++) {
                var y = d * 2 + 1;
                if (!(dungeonOBJ.cell[x][y] & ROOM))
                    if (!((itr_a === 0 || d === 0) && random(2) > 0)) {
                        var g = {
                            i: itr_a,
                            j: d,
                            size: index[roomSizeCurrent]['key']
...
function check_sill(cells, room, x, y, wall) {
    ...
    if (out_cell & ENTRANCE) return false;
...
function empty_points_from_space(dungeonOBJ, open_space, rooms) {
    ...
    var points = [];
    open_space = Math.max( 3, open_space );
    if (open_space % 2 === 1) open_space++;
    var offset = Math.max(2, Math.round(open_space / 2));
    ...
    for (let x = 1; x <= (dungeonOBJ.max_col - open_space); x++) {
        for (let y = 1; y <= (dungeonOBJ.max_row - open_space); y++) {
            ...
            if (!explored[x][y]) {
                if (dungeonOBJ.cell[x][y] === NOTHING) {
                    ...
                    if (success !== false) {
                        ...
                        var room = {
                            ...
                            'door': {
                                south: [
                                    //new Door(x+offset+di.south, y+offset+dj.south, x+offset, y+offset, ENTRANCE, 'open', 'Door', 0, 'south',false)
                                ]
                            }
                        ...
                        try {
                            rooms.push(room);
                            open_door(dungeonOBJ, room, {
                                sill_r: x + offset + di.south,
                                sill_c: y + offset + dj.south,
                                door_r: x + offset,
                                door_c: y + offset,
                                dir: 'south',
                                out_id: 0
                            ...
                            console.error('error registering faux door sills: ' + err);
                            break;
                        }
                    }
                }
                ...
                console.error('Error at [' + x + ',' + y + '] testing explored initially');
...
function corridors_random(dungeonOBJ){
    ...
    //iterate over the dungeon cells and fire off crazy tunnel commands
    for (var x2 = 1; x2 < dungeonOBJ.n_i; x2++) {
        ...
        for (y2 = 1; y2 < dungeonOBJ.n_j; y2++) {
            ...
            //var canTunnelHere = ! (dungeonOBJ.cell[x][y] & CORRIDOR );
            //if (dungeonOBJ.room_layout !== 'dense' && dungeonOBJ.room_layout !== 'tight' ) 
            //    canTunnelHere = canTunnelHere || !(dungeonOBJ.cell[x][y] & ROOM) ;
            //if ( canTunnelHere )
            //    dungeonOBJ = tunnel(dungeonOBJ, x2, y2);
        }
    }
    return dungeonOBJ;
}
...
function open_tunnel(dungeonOBJ, i, j, direction) {
    ...
    var e = (x + i) / 2;
    var f = (y + j) / 2;
    var soundTunnelSuccess = sound_tunnel(dungeonOBJ, e, f, i, j);
    if (soundTunnelSuccess)
        return delve_tunnel(dungeonOBJ, x, y, i, j);
    else
        return false;
}
...
function sound_tunnel(dungeonOBJ, e, f, i, j) {
    ...
    for (j = x[0]; j <= x[1]; j++)
        for (i = y[0]; i <= y[1]; i++) {
            ...
            //var is_blocked = dungeonOBJ.cell[j][i] & BLOCK_CORR;
            //if (dungeonOBJ.room_layout !== 'dense') is_blocked = is_blocked || dungeonOBJ.cell[j][i] & ROOM;
...
function dungeon_to_astar_graph(dungeonOBJ) {
    ...
    var grid = dungeonOBJ.cell.map((subArray) => {
        var squiggle = Math.trunc( (random(dungeonOBJ.straight_pct) - 50)/5 );
        return subArray.map((yval) => {
            ...
            if (yval & CORRIDOR) return Math.min(Math.max( 10 + squiggle, 2),50);
            if (yval & DOORSPACE) return 1;
            if (yval === NOTHING) return Math.min(Math.max(25 + squiggle, 3),50);
...
function check_tunnel(cell, y, x, stair_end_dir) {
    var e = true,
        g;
    if (stair_end_dir) {
        if (stair_end_dir.corridor) {
                g = stair_end_dir.corridor;
                g.each(function(f) {
                    if (cell[y + f[0]])
        ...
        if ( stair_end_dir.walled) {
            g = stair_end_dir.walled;
            g.each(function(f) {
                if (cell[y + f[0]])
...
function gelatinous_cube(dungeonOBJ) {
    ...
    if (dungeonOBJ.remove_deadends)
        dungeonOBJ.close_arcs = dungeonOBJ.remove_pct;
...
function collapse(dungeonOBJ, x, y, close_end) {
    ...
    $H(close_end).keys().each(function(key) {
        if (check_tunnel(e, x, y, close_end[key])) {
            var f;
            if (close_end[key].close) {
                f = close_end[key].close;
                f.each(function(h) {
                    e[x + h[0]][y + h[1]] = NOTHING;
                });
            }
            if (close_end[key].open) {
                f = close_end[key].open;
                e[x + f[0]][y + f[1]] |= CORRIDOR;
            }
            if (close_end[key].recurse) {
                 direction = close_end[key].recurse;
                 dungeonOBJ = collapse(dungeonOBJ, x + direction[0], y + direction[1], close_end);
            }
        }
    });
    dungeonOBJ.cell = e;
...
function image_dungeon_setup(dungeonOBJ) {
    ...
    //image_to_base64(dungeonOBJ.Dungeon_Base_Image, $('gm_map'));

    dungeonOBJ.Dungeon_Image.drawImage(
        dungeonOBJ.Dungeon_Base_Image.canvas,
        dungeonOBJ.Dungeon_Base_Image.canvas.width,
        dungeonOBJ.Dungeon_Base_Image.canvas.height
    );
    ...
    //image_to_base64(dungeonOBJ.Dungeon_Image, $('player_map'));
}
...
function image_dungeon_player(dungeonOBJ) {
    ...
    $j('img#blank_map')[0].setAttribute('src', dungeonOBJ.Dungeon_Base_Image.canvas.toDataURL());
}
...
function image_dungeon_gm(dungeonOBJ) {
    ...
    $j('img#gm_map')[0].setAttribute('src', dungeonOBJ.Dungeon_Image.canvas.toDataURL());
}
...
function base_layer(image, mapdetails, pallet_color) {
    ...
    if (palette.open_grid !== undefined && palette.open_grid !== null) {
        bgColor = palette.open_grid;
        image_grid(image, mapdetails, bgColor, context);
    }
    else if (palette.grid !== undefined && palette.grid !== null) {
        bgColor = palette.grid;
        image_grid(image, mapdetails, bgColor, context);
    }
    return canvas;
}
 ...
 * 
 * @param {Object} image
 * @param {any} drawSettings
 * @param {any} bgColor
 * @param {CanvasRenderingContext2D} context
 */
function image_grid(image, drawSettings, bgColor, context) {
    if (image.grid !== 'none')
        if (image.grid === 'hex') hex_grid(drawSettings, bgColor, context);
        else image.grid === 'vex' ? vex_grid(drawSettings, bgColor, context) : square_grid(image, drawSettings, bgColor, context);
    return true;
}
...
function hex_grid(drawSettings, bgColor, context) {
    var offset_v = drawSettings.cell_size/2;
    var offset_h = offset_v / 3.4641016151;
    offset_v = offset_v / 2;
    var g = drawSettings.width / (3 * offset_h);
    drawSettings = drawSettings.height / offset_v;
    var f;
    for (f = 0; f < g; f++) {
        var h = f * 3 * offset_h,
            i = h + offset_h,
            j = h + 3 * offset_h,
            k;
        for (k = 0; k < drawSettings; k++) {
            var l = k * offset_v,
                o = l + offset_v;
            if ((f + k) % 2 !== 0) {
                draw_line(context, h, l, i, o, bgColor);
                draw_line(context, i, o, j, o, bgColor);
            } else draw_line(context, i, l, h, o, bgColor);
        }
    }
    return true;
}

function vex_grid(b, c, d) {
    var e = b.cell_size/2;
    var a = e / 2;
    e = e / 3.4641016151;
    var g = b.width / a;
...
function fill_image(dungeonOBJ, map_details, dungeon_image) {
    ...
    if (palette.fill_hatching === 'hatch') {
        ...
        fill = palette.fill;
        fill ? 
            fill_rect(dungeon_image, 0, 0, d, e, fill) 
            : fill_rect(dungeon_image, 0, 0, d, e, palette.black);
        //if (fill !== null) 
        //    fill_rect(dungeon_image, 0, 0, d, e, fill);
    }
    if (palette.fill_grid) {
        fill = palette.fill_grid;
        image_grid(dungeonOBJ, map_details, fill, dungeon_image);
    }
    else if (palette.grid) {
        fill = palette.grid;
        image_grid(dungeonOBJ, map_details, fill, dungeon_image);
    }
    return true;
...
function image_walls(dungeonOBJ, map_details, dungeon_image) {
    ...
    var x;
        ...
        for (x = 1; x <= dungeonOBJ.n_rows; x++) {
            var x_offset = x * rez,
                x_offset_p = x_offset + rez,
                y;
            for (y = 1; y <= dungeonOBJ.n_cols; y++)
                if (dungeonOBJ.cell[x][y] & OPENSPACE) {
                    var y_offset = y * rez,
 ...
 */
function image_room_labels(dungeon, map_details, canvas) {
    var r;
    var hotspot_radius = map_details.cell_size*2;
    for (r = 0; r <= dungeon.n_rows; r++) {
        var c;
        for (c = 0; c <= dungeon.n_cols; c++)
            if (dungeon.cell[r][c] & OPENSPACE) {
                var label = cell_label(dungeon.cell[r][c]);
                if (label) {
                    image_text(r, c, canvas, label, map_details);
                    var area = document.createElement('area');
                    $(area).setAttribute('shape', 'circle');
                    $(area).setAttribute('coords', r + ',' + c + ',' + hotspot_radius);
                    $(area).setAttribute('alt', 'Room #' + label);
                    map_details.cell_size * r, map_details.cell_size * c;
                    $('dungeon_map').appendChild(area);
                }
            }
    }
    return true;
 ...
 * @param {number} cell_x x coordinate to start at
 * @param {number} cell_y y coordinate to start at
 * @param {CanvasRenderingContext2D} image image object to write on
...
function save_map() {
    if ($('map_div').getAttribute('class') === 'hidden') {
        save_image($('blank_map'), $('dungeon_name').getValue() + '_blank_map' + '.png');
    } else {
        save_image($('gm_map'), $('dungeon_name').getValue() + '_gm_map' + '.png');
    }
}

function swap_map() {
    if ($('map_div').getAttribute('class') === 'hidden') {
        $('map_div').setAttribute('class', '');
        $('playermap_div').setAttribute('class', 'hidden');
    } else {
        $('map_div').setAttribute('class', 'hidden');
        $('playermap_div').setAttribute('class', '');
    }
}

function do_print() {
    var userResolution = parseInt( $('Resolution').getValue());
