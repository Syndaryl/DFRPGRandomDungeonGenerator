...
function build_tree(dungeonOBJ) {
    ...
    //var junction_points = shuffle(empty_points_from_space(dungeonOBJ, room_spacing, rooms).map(t => [t.X, t.Y]));

    ////console.warn('delaunator points from empty spaces: ' + JSON.stringify(junction_points, null, '   '));

    //var selected_junctions = junction_points.slice(0, Math.max(Math.round(junction_points.length / 5), 1));
    //dungeonOBJ.junction_points = selected_junctions;

    //console.info('delaunator points subset of empty spaces for union: ' + JSON.stringify(selected_junctions, null, '   '));
