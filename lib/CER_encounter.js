...
function monster_room(dungeonOBJ, roomIDX, threat_table) {
    ...
    if (has_monster) {
        ...
        var dungeon_threat_level = threat_distribution[dungeonOBJ.challenge];
        
        var challenge = select_from_weighted_table(dungeon_threat_level);
        
        target_cer *= rand_between(threats[challenge.Description].NgtX, threats[challenge.Description].NltX);
        target_cer = Math.max(lowest_cer_available, target_cer);

        var room_area = dungeonOBJ.room[roomIDX].width * dungeonOBJ.room[roomIDX].height; // units: feet, square feet
        
        var qualifying_monsters_table = monsters.filter(function (monster) {
            return monster.CER <= target_cer;
        });
        ...
        if (qualifying_monsters_table == undefined || qualifying_monsters_table.length == 0)
        {
            ...
            console.trace();
            dungeonOBJ.room[roomIDX].monsters.push(dummy_monster(target_cer));
            return room;
        }

        var largest_SM_obj = get_happy_sm(room_area); // largest SM that wants to live in this room

        //var qualifying_SM_monsters_table = qualifying_monsters_table.filter(function (monster) {
        //    return monster.SM <= largest_SM_obj.Size;
        //});
        ...
        if (encounter_monster == undefined || encounter_monster == {} || encounter_monster.CER == undefined)
        {
        ...
        //var quantity = Math.max( // at least one
        //    1, Math.min( // but only as many as really fit
        //        Math.floor(room_area/largest_SM_obj.Area),
        //        Math.round(target_cer / encounter_monster.CER)
        //    )
        //); 

        console.log(
            "This is Room " + roomIDX + "; it is " + room['width'] + " x " + room['height'] + " feet (" + room_area + " sq. ft.) The largest SM that wants to live here is "+ largest_SM_obj.Size +"\n" +
            "This is a " + challenge.Description +  " encounter. " +
        ...
        dungeonOBJ.room[roomIDX]['threat'] = challenge.Description;
    }
    return room
}
...
{
    var possible = SizeArea.filter(function (x) { return x.Area <= area }).sort(function (a, b) { a.Area < b.Area });
