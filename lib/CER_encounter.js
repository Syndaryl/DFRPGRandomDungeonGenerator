 ...
 */
function monster_room(dungeonOBJ, roomIDX, threat_table, clear = true, monster_list) {
    if (typeof (monster_list) == "undefined") {
    ...
    if (has_monster) {
        ...
        dungeonOBJ.room[roomIDX] = add_monster(roomIDX, threat_table, encounter_scale.cer, dungeonOBJ.room[roomIDX], monster_list);
    ...
    console.log("final chosen monsters for room " + roomIDX + ": " + JSON.stringify(
        //dungeonOBJ.room[roomIDX].monsters
        dungeonOBJ.room[roomIDX].monsters.map(x => x.monster.Description + " (" + x.monster.Environment + ") x " + x.quantity)
        ));
...
/**
 * 
 * @param {Array} first list of strings
 * @param {Array} second list of strings
 * @returns {Array} intersection between the two lists.
 */
function intersect_safe_strings(a, b) {
    var ai = bi = 0;
    var result = [];

    while (ai < a.length && bi < b.length) {
        if (a[ai].localeCompare(b[bi]) < 0) { ai++; }
        else if (a[ai].localeCompare(b[bi]) > 0) { bi++; }
        else /* they're equal */ {
            result.push(ai);
            ai++;
            bi++;
 ...
 * @param {Array} biome_list
 * @param {Array} monster_table
 */
function filter_monsters_by_biome(dungeonOBJ, biome_list, monster_table) {
    var results = monster_table.filter(
        function (monster) {
            var environments = [];
            environments = monster.Environment.toUpperCase().split("\. +");
            environments.push("ANY");
            //biome_list.push("ANY");
            if (intersect_safe_strings(biome_list, environments).length > 0) {
                return true;
            }
            return false;
        }
