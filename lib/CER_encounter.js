...
function wandering_monsters(dungeonOBJ)
{
    ...
    //used_monsters.sort(alphabetical_description);
        unique_monsters.forEach( 
            function (value, key, map){
                ...
                var mon = key;
                if (mon.CER <= dungeonOBJ.cer * 1.5 && mon.CER >= dungeonOBJ.cer * 0.15 && mon.SM < 3 )
                {
    ...
    while (used_monsters.length < 5) {
        var extra_mon = get_monster(2, dungeonOBJ.cer*1.25, dungeonOBJ.cer*0.75);
        extra_mon.Weight = 1;
