...
function monster_room(dungeonOBJ, roomIDX, threat_table) {
    ...
    if (has_monster) {
        ...
        var quantity = Math.min(1, Math.round(rand_between(min_cer, max_cer) / monster.CER)); // at least one

        console.log(
            "This is a " + threat["Description"] + " " + (solo ? "solo" : "group") + " encounter. \n" +
            "The target CER is " + target_cer + "\n" +
            "Our N is " + n + ", the min_cer is " + min_cer + " and the max_cer is " + max_cer + "\n" +
            "The selected monster is " + monster.Description + " (CER " + monster.CER + ")" + "\n" +
