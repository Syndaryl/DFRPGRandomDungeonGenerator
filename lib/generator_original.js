 ...
 * Weighted tables are an Array of Objects with the Weight property, where Weight is a number.
 * If 
 * @param {weighted_table_entry[]} tbl
 */
function select_from_weighted_table(table) {
    if (table[0].hasOwnProperty('Weight')) {
        var tableLength = table.reduce(function (a, b) { return a + b.Weight; }, 0);
        console.log('table has a total weight length of ' + tableLength);
        var roll = random(tableLength);
		// TODO: Look up roll result in table, return that.
    }
    return ""
}
