...
function experimental_rooms(dungeonOBJ) {
    ...
    return tight_rooms(dungeonOBJ, 9);
...
function tight_rooms(dungeonOBJ, pack_factor) {
    ...
    for (var roomSizeCurrent = roomMax; roomSizeCurrent >= 0; roomSizeCurrent--){ // for each room size
        roomCount = alloc_rooms(dungeonOBJ, dungeon_options.room_size[index[roomSizeCurrent]], pack_factor); // figure out how many of this size we can fit
        roomCount = Math.max(1, roomCount - 2); // tweak
