 ...
 * @param {*} obj The object to be paramaterized
 * @returns {string} The paramaterized version of the object
 */
...
function init_dungeon() {
    ...
    c = c.mask
    if (c) dungeonOBJ = mask_cells(dungeonOBJ, c);
    ...
    if (room.complex) room.complex.each(function(k) {
        ...
        if (edgeNorth >= 3) {
            for (j = edgeWest; j <= edgeEast; j += 2) 
                check_sill_and_push(cell, room, edgeNorth, j, "north");
        }
        if (edgeSouth <= dungeon.n_rows - 3)
            for (j = edgeWest; j <= edgeEast; j += 2)
                check_sill_and_push(cell, room, edgeSouth, j, "south");
        if (edgeWest >= 3)
            for (j = edgeNorth; j <= edgeSouth; j += 2)
                check_sill_and_push(cell, room, j, edgeWest, "west");
        if (edgeEast <= dungeon.n_cols - 3)
            for (j = edgeNorth; j <= edgeSouth; j += 2)
                check_sill_and_push(cell, room, j, edgeEast, "east");
    }
    return exits;

    function check_sill_and_push(c, r, e, k, d) {
        var doors = check_sill(c, r, e, k, d);
        if (doors)
            exits.push(doors);
    }
}
...
function check_tunnel(cell, y, x, stair_end_dir) {
    ...
    if (stair_end_dir.walled) {
        stair_end_dir = stair_end_dir.walled;
...
function collapse(dungeonOBJ, x, y, close_end) {
    ...
    $H(close_end).keys().each(function(g) {
        if (check_tunnel(e, x, y, close_end[g])) {
            var f = close_end[g].close;
            if (f) f.each(function(h) {
                e[x + h[0]][y + h[1]] = NOTHING;
            });
            f = close_end[g].open;
            if (f) e[x + f[0]][y + f[1]] |= CORRIDOR;
            g = close_end[g].recurse;
            if (g) dungeonOBJ = collapse(dungeonOBJ, x + g[0], y + g[1], close_end);
        }
    });
    dungeonOBJ.cell = e;
    return dungeonOBJ
...
function get_palette(dungeonOBJ) {
    ...
    //     : palette.standard;
    var c = palette_loc.colors;
    if (c) $H(c).keys().each(function(d) {
        palette_loc[d] = c[d];
    });
...
function base_layer(a, b) {
    ...
    var d = c.getContext("2d"),
        ...
        palette = b.palette,
        h = palette.open;
    (h) ? fill_rect(d, 0, 0, e, g, h) : fill_rect(d, 0, 0, e, g, palette.white);
    h = palette.open_grid;
    if (h) image_grid(a, b, h, d);
    else {
        h = palette.grid;
        if (h) image_grid(a, b, h, d);
    }
...
function fill_image(dungeonOBJ, map_details, dungeon_image) {
    var d = map_details.max_x,
        ...
        f = palette.fill;
    (f) ? fill_rect(dungeon_image, 0, 0, d, e, f): fill_rect(dungeon_image, 0, 0, d, e, palette.black);
    if (f) fill_rect(dungeon_image, 0, 0, d, e, f);
    f = palette.fill_grid;
    if (f) image_grid(dungeonOBJ, map_details, f, dungeon_image);
    else {
        f = palette.grid;
        if (f) image_grid(dungeonOBJ, map_details, f, dungeon_image);
    }
