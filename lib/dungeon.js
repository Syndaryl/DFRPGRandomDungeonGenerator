...
function door_sills(dungeon, room) {
    ...
    if (typeof room.complex == Array) room.complex.each(function(sub_room) {
        ...
        if (edgeSouth <= dungeon.n_rows - 3)
            for (j = edgeWest; j <= edgeEast; j += 2)
                if ((doors = check_sill(cell, room, edgeSouth, j, "south")) != null) exits.push(doors);
        if (edgeWest >= 3)
            for (j = edgeNorth; j <= edgeSouth; j += 2)
                if ((doors = check_sill(cell, room, j, edgeWest, "west")) != null) exits.push(doors);
        if (edgeEast <= dungeon.n_cols - 3)
            for (j = edgeNorth; j <= edgeSouth; j += 2)
                if ((doors = check_sill(cell, room, j, edgeEast, "east")) != null) exits.push(doors);
    }
    return exits;
}
