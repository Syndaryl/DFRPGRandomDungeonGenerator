...
/**
 * @returns {Array<string>} Sorted, trimmed list of environments used
 * */
function get_environments() {
    var envIDX = 0;
    var envSubIDX = 0;
    // Temporary: Find out how many monster environments we actually have.
    var environments_array_array = monsters.map(x => x.Environment.split(".").map(y => y.trim().toUpperCase()));

    var environments_list = [""];
    environments_list = environments_list.map(x => x === ""? "ANY" : x);
    for (envIDX = 0; envIDX < environments_array_array.length; envIDX++) {
        environments_list = environments_list.concat(environments_array_array[envIDX]);
    }

    environments_list = environments_list.filter(function (v, i, self) {
        // It returns the index of the first instance of each value 
        return i === self.indexOf(v);
    });

    environments_list = environments_list.sort();

    if (environments_list[0] === "")
        delete environments_list[0];

    return environments_list;
}

/**
 * Populates and initializes the webforms drop-down options, and sets up event observers
 */
function init_form() {
    $("dungeon_name").observe("change", do_namechange_reaction);
    $("new_name").observe("click", new_name);
    $("cer").observe("change", new_dungeon);
    $("cer").observe("change", save_dungeon_configuration);
    ...
    $("resolution").observe("change", save_dungeon_configuration);
    $("remote_treasure").observe("change", new_dungeon);
    $("biomes_fields").observe("change", biome_ticky_change);

    setup_biomes();
    populate_dropdown_lists();
    set_defaults();
    do_namechange_reaction();
    update_self_link();

    /**
     * populate_dropdown_lists() goes through the dungeon options and 
     * uses the property list of each option to initialize the dropdown lists
     * */
    function populate_dropdown_lists() {
        console.log(JSON.stringify(dungeon_options));
        $H(dungeon_options).keys().each(function(key_primus) {
            console.log(key_primus);
    ...
    /**
     * set_defaults() goes through the settings boxes and sets everything to 
     * default values.
     * @returns null
     * */
    function set_defaults() {
        LoadFromStorage();
        if (Global_Dungeon_Config.seed && Global_Dungeon_Config.seed !== "") {
            if (! Global_Dungeon_Config.dungeon_name)
                $("dungeon_name").setValue(Global_Dungeon_Config["seed"]);
        }
        else
            new_name();
        var query_keys = $H(Global_Dungeon_Config).keys();
        query_keys.each(set_default);
        if (Global_Dungeon_Config.resolution)
            $("resolution").setValue(Global_Dungeon_Config.resolution);
        Global_Dungeon_Config.cer = $("cer").getValue();
        $H(dungeon_options).keys().each(function(key_primus) {
         ...
         */
        function set_default(element_id) {
            if (!(element_id in Global_Dungeon_Config)) {
                alert(element_id + " is not found in the Global_Dungeon_Config string. Odd.");
                return;
            }
            if ($(element_id) === null) {
                alert(element_id + " is not a valid control. Odd.");
                return;
            }
            var default_value = Global_Dungeon_Config[element_id];
            
            if (default_value === undefined) {
                alert(element_id + " has an undefined default value. Odd.");
                return;
            }
            
            var tag_type = $(element_id).tagName.toLowerCase();
            if (tag_type === "select") {
                $(element_id).setValue(default_value);
            } else if (tag_type === "input") {
                $(element_id).setValue(default_value);
            } else if (tag_type === "fieldset") {
                default_value = JSON.parse(default_value);
                var tags = $(element_id).getElementsByTagName('input');
                for (var x = 0; x < tags.length; x++) {
                    tags.item(x).checked = false;
                }
                for (var y = 0; y < default_value.length; y++) {
                    tags.namedItem(default_value[y]).checked = true;
                }
            }
    ...
    function setup_biomes() {
        var available_environments = get_environments();
        available_environments.each(function(env) {
            var labelText = ucfirst(lc(env));
            var li = jQ('<p/>')
                .attr('class', 'dropdown-tickybox')
                .appendTo($("biomes_fields"));

            var input = jQ('<input/>')
                .attr('type', 'checkbox')
                .attr('name', 'biome_list')
                .attr('value', env)
                .attr('id', env)
                .attr('class', 'dropdown-tickybox')
                //.insert()
                //.appendTo($("biomes_fields"));
                .appendTo(li);
            input.on("change", new_dungeon);
            input.on("change", save_dungeon_configuration);
            input.on("change", biome_ticky_change);
 ...
 */
function do_namechange_reaction() {
 ...
 */
function new_name() {
    var new_name = generate_text("Dungeon Name");
    $("dungeon_name").setValue(new_name, gen_data);
    Global_Dungeon_Config.name = new_name;
    Global_Dungeon_Config.seed = new_name;
    cer_reaction();
}
...
/**
 * */
function biome_ticky_change() {
    var tickies = getCheckedBiomes();
    dungeon_options.biome_list = tickies;
    //alert(JSON.stringify(tickies));
}

function getCheckedBiomes() {
    var tickies = [];
    var checked_controls = jQ("input[name='biome_list']:checked");
    for (var x = 0; x < checked_controls.length; x++)
        tickies.push(checked_controls[x].getValue());
    return tickies;
}
...
function update_self_link() {
    ...
    var options = new dungeon_configuration(default_query);
    $H(dungeon_options).keys().each(function (field) {
        if ($(field) != null) { // update from control value
    ...
    var arr = getCheckedBiomes();
    options.biomes_fields = arr ? JSON.stringify(arr) : JSON.stringify(['ANY']);
    options.cer = $("cer").getValue() || 125;
    options.resolution = $("resolution").getValue() || 25;

    Global_Dungeon_Config = Global_Dungeon_Config.update(options);
 ...
 */
function save_dungeon_configuration(set_item_key = 'last_saved_query') {
    var dungeon_options_to_save = new dungeon_configuration().update(Global_Dungeon_Config);
    dungeon_options_to_save.seed = dungeon_options_to_save.seed || $("dungeon_name").getValue();
    //var checkboxes = document.getElementsByName("favorite_pet");
    //dungeon_options_to_save["biome_list"] = $("biome_list").getValue();
    var arr = jQ('biome_list').map(function () {
            return this.value;
    }).get(); 
    dungeon_options_to_save.biome_list = JSON.stringify(arr);
    dungeon_options_to_save.resolution = dungeon_options_to_save.resolution || $("resolution").getValue();
...
function resolution_change() {
    ...
    Global_Dungeon.resolution = Global_Dungeon.cell_size;
    Global_Dungeon_Config.resolution = Global_Dungeon.cell_size; 
    Global_Dungeon_Config.cell_size = Global_Dungeon.cell_size;
...
/**
 * 
 * @param {Array} arr The object with all the dungeon settings, with generated rooms
 * @returns {Array} Returns the dungeonOBJ with flavor text, features, light, mana, sanctity, hydration, monsters, and traps.
 */
...
function populate_rooms(dungeonOBJ) {
    ...
    var filtered_monster_list = filter_monsters_by_biome(dungeonOBJ, dungeon_options.biome_list, monsters);
    ...
    for (roomIDX = 1; roomIDX <= dungeonOBJ.n_rooms; roomIDX++) {
        ...
        dungeonOBJ.room[roomIDX] = monster_room(dungeonOBJ, roomIDX, threat_distribution[dungeonOBJ.challenge], true, filtered_monster_list);
...
/**
 * Gets tables by name, returns table objects.
...
/**
 * creates the finalized dungeon object, ready to send to the mapper
 * @returns {Dungeon} returns the finalized dungeon object, ready to send to the mapper
 * */
...
function init_dungeon() {
    ...
    $H(Global_Dungeon_Config).keys().each(function(g) {
        dungeonOBJ[g] = Global_Dungeon_Config[g];
    });
    //dungeon_options.cer = Global_Dungeon_Config.cer;
    var b = get_dungeon_configuration("dungeon_size", dungeonOBJ),
        c = get_dungeon_configuration("dungeon_layout", dungeonOBJ),
        d = b.size,
        e = c.aspect,
        cellsize = Global_Dungeon_Config.resolution;
...
function open_cells(dungeonOBJ, map_details, dungeon_image) {
    ...
    var row;
    for (row = 0; row <= dungeonOBJ.n_rows; row++) {
        var row_pixel = row * resolution,
            column;
        for (column = 0; column <= dungeonOBJ.n_cols; column++)
            if (dungeonOBJ.cell[row][column] & OPENSPACE) {
                var column_pixel = column * resolution;
                dungeon_image.drawImage(map_details, column_pixel, row_pixel, resolution, resolution, column_pixel, row_pixel, resolution, resolution);
            }
    }
    return true;
}
...
function image_doors(dungeonOBJ, map_details, canvas) {
     ...
     * @param {object} map_details the map details object
     * @param {any} canvas - HTML canvas object
     * @returns {null} nothing
     */
function draw_door(dungeonOBJ, door_object, map_details, canvas) {
    ...
    var left_is_open = dungeonOBJ.cell[y_cell][x_cell - 1] & OPENSPACE;
    var y_pix2 = y_pix + full;
    var x_pix2 = x_pix + full;
    var y_ = Math.floor((y_pix + y_pix2) / 2),
        x_ = Math.floor((x_pix + x_pix2) / 2);
    if (door_attributes.wall) left_is_open ?
        draw_line(canvas, x_, y_pix, x_, y_pix2, c_wall) :
        draw_line(canvas, x_pix, y_, x_pix2, y_, c_wall);


    if (door_attributes.arch) // all nonsecret doors include an archway
                              // Two rectangles are drawn to frame the door space (and door sign if any)
        if (left_is_open) {
            if (door_object.double_wide) {
                fill_rect(canvas, x_ - 1, y_pix, x_ + 1, y_pix + short, c_wall);
                fill_rect(canvas, x_ - 1, y_pix2 - short, x_ + 1, y_pix2, c_wall);
            }
            else {
                fill_rect(canvas, x_ - 1, y_pix, x_ + 1, y_pix + short, c_wall);
                fill_rect(canvas, x_ - 1, y_pix2 - short, x_ + 1, y_pix2 + full, c_wall);
            }
        } else {
            fill_rect(canvas, x_pix, y_ - 1, x_pix + short, y_ + 1, c_wall);
            fill_rect(canvas, x_pix2 - short, y_ - 1, x_pix2 + full, y_ + 1, c_wall);
        }
    //if (door_attributes.door) is_open ?
    //    stroke_rect(canvas, x_ - mid, y_pix + short + 1, x_ + mid, y_pix2 - short - 1, c_door) :
    //    stroke_rect(canvas, x_pix + short + 1, y_ - mid, x_pix2 - short - 1, y_ + mid, c_door);
    //if (door_attributes.lock) is_open ?
    //    draw_line(canvas, x_, y_pix + short + 1, x_, y_pix2 - short - 1, c_door) :
    //    draw_line(canvas, x_pix + short + 1, y_, x_pix2 - short - 1, y_, c_door);
    ...
    //    if (is_open)
    //        for (y_pix = y_pix + short + 2; y_pix < y_pix2 - short; y_pix += 2) set_pixel(canvas, x_, y_pix, c_door);
    //    else
    //        for (y_pix = x_pix + short + 2; y_pix < x_pix2 - short; y_pix += 2) set_pixel(canvas, y_pix, y_, c_door);
...
var Global_Dungeon = new Dungeon();