...
/*
NgtX	Name	Treasure	Description
11	Nuisance	 0.25 	The monsters are pests or a diversion of some kind.
3	Fodder	 0.5 	The enemy may wear down the PCs, but will rarely beat them.
0.50	Worthy	 1 	This fight is roughly even.
0.10	Boss	 5 	The enemy outmatches the party, who will need luck and quick wits to win.
0	Epic	 20 	Only luck, emergency one-use magic, or unexpected reinforcements will save the heroes!
*/
...
function cash_value(target) {
    ...
    result += '[' + gurps_money(net) + '; '  + gurps_weight(weight)  + ']';
    return result;
}
...
function image_dungeon_shared(dungeon) {
...
function image_dungeon_player(dungeon) {
    var map_details = scale_dungeon(dungeon),
        dungeon_image = new_image(map_details.width, map_details.height),
        palette = get_palette(map_details);
    map_details.palette = palette;
    palette = base_layer(dungeon, map_details, dungeon_image);
    map_details.base_layer = palette;
    fill_image(dungeon, map_details, dungeon_image);
    open_cells(dungeon, map_details, dungeon_image);
    image_walls(dungeon, map_details, dungeon_image);
    dungeon.door && image_doors(dungeon, map_details, dungeon_image);
    image_labels(dungeon, map_details, dungeon_image);
    dungeon.stair && image_stairs(dungeon, map_details, dungeon_image)
}
