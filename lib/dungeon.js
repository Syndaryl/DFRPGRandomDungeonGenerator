...
function get_door_properties (e){
    if (e == ARCH) {
        dungeonOBJ.cell[door_row][door_column] |= ARCH;
        door_obj.key = "arch";
        door_obj.type = "Archway"
    } else if (e == DOOR) {
        dungeonOBJ.cell[door_row][door_column] |= DOOR;
        door_obj.key = "open";
        door_obj.type = "Unlocked Door"
    } else if (e == LOCKED) {
        dungeonOBJ.cell[door_row][door_column] |= LOCKED;
        door_obj.key = "lock";
        door_obj.type = "Locked Door"
    } else if (e == TRAPPED) {
        dungeonOBJ.cell[door_row][door_column] |=
            TRAPPED;
        door_obj.key = "trap";
        door_obj.type = "Trapped Door"
    } else if (e == SECRET) {
        dungeonOBJ.cell[door_row][door_column] |= SECRET;
        door_obj.key = "secret";
        door_obj.type = "Secret Door"
    } else if (e == PORTC) {
        dungeonOBJ.cell[door_row][door_column] |= PORTC;
        door_obj.key = "portc";
        door_obj.type = "Portcullis"
    }
}
...
function open_door(dungeonOBJ, room, candidate) {
    ...
    var e = select_from_table(doors_table);
    var door_obj = {
        row: door_row,
        col: door_column
    };
