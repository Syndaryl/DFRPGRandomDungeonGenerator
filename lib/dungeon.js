...
$(function () {
    ...
    $.each(dungeon_options,function (key_primus) {
        $.each(dungeon_options[key_primus],function (key_secondus) {
            var option_text = dungeon_options[key_primus][key_secondus].title;
            $('#' + key_primus).append(new Option(option_text, key_secondus));
        });
    });

    // set defaults
    $.each(default_query, set_default);
    if (default_query["seed"] && default_query["seed"] != "")
        $('#dungeon_name').val(default_query["seed"]);
    else
        new_name();
    if (urlParams.resolution)
        $('#Resolution').val(urlParams.resolution);

    dungeon_options.cer = $('#cer').val();
    name_reaction();

    // observe changes to the text boxes and buttons and stuffs and junk.
    $('#dungeon_name').on("change", name_reaction);
    $('#new_name').on("click", new_name);
    $('#cer').on("change", new_dungeon);
    $('#generosity').on("change", new_dungeon);
    $('#travel_distance').on("change", new_dungeon);
    $('#challenge').on("change", new_dungeon);
    $('#Resolution').on("change", resolution_change);
    $('#remote_treasure').on("change", new_dungeon);

    //  observe the dropdowns
    $.each(dungeon_options, function (key_primus) {
        $(key_primus).bind("change", new_dungeon);
        $(key_primus).bind("change", save_dungeon_configuration);
    });
});
...
function update_self_link() {
    ...
    var options = {};
    $.each(dungeon_options,function (field) {
        if ($('#' + field) != null) {
            var value = $('#' + field).val();
            options[field] = value;
        }
    ...
    options['seed'] = $('#dungeon_name').val();
    options['resolution'] = $('#Resolution').val() || 25;
...
function name_reaction() {
    $('#dungeon_title').val($('#dungeon_name').val());
 ...
 */
function new_name() {
    $('#dungeon_name').val(generate_text("Dungeon Name"), gen_data);
    cer_reaction();
}
...
function save_dungeon_configuration(set_item_key) {
    ...
    $.each(dungeon_options,function (key_primus) {
        dungeon_options_to_save[key_primus] = $('#' + key_primus).val();
    });
...
function resolution_change() {
    console.log("Resolution change to " + $('#Resolution').val());
    Global_Dungeon.cell_size = parseInt($('#Resolution').val());
...
function html_debug_dump(element, data) {
    ...
    pre.innerText = JSON.stringify(data, undefined, '   ');
...
function inform_door(mask, penalty) {
    var door_text = '';
    if (mask & DOOR) { door_text = door_text + select_from_weighted_table(doorsList).Description; }
    else if (mask & PORTC) { door_text = door_text + select_from_weighted_table(portcullisesList).Description; }
    else if (mask & ARCH) { door_text = door_text + select_from_weighted_table(archesList).Description; }
    ...
    if (mask & STUCK) { door_text = door_text + '; ' + select_from_weighted_table(stuckDoors).Description; }

    if (mask & TRAPPED) { door_text = door_text + '; ' + 'it is trapped' + '; ' + select_from_weighted_table_tagged(traps, "Door").Description; }

    if (mask & SECRET) { door_text = door_text + '; ' + select_from_weighted_table(concealment).Description; }
...
function html_wandering_monsters(element, wandering_monsters, target_cer) {
    ...
    while (accumulate <= 100) {
        ...
        } else if (quantity === 4) { quantity = '1d6-2 (min 1)'; }
        else if (quantity === 3) { quantity = '1-3'; }
        else if (quantity === 2) { quantity = '1-2'; }
...
function create_dungeon() {
    ...
    //dungeonOBJ.mst_structure = build_tree(dungeonOBJ);
...
function init_dungeon() {
    ...
    dungeonOBJ.seed = init_seed($('#dungeon_name').val());
    $.each(dungeon_options,function (g) {
        dungeonOBJ[g] = $('#' + g).val();
    });
    dungeon_options.cer = $('#cer').val();
    var b = get_dungeon_configuration("dungeon_size", dungeonOBJ),
        ...
        d = b.size,
        e = c.aspect,
        cellsize = parseInt($('#Resolution').val());
...
function emplace_rooms(dungeonOBJ) {
    ...
    switch (dungeonOBJ.room_layout) {
        ...
        case "scatter":
            return scatter_rooms(dungeonOBJ, 10);
        case "sparse":
            return scatter_rooms(dungeonOBJ, 18);
        case "experimental":
...
function cruciform_layout_rooms(dungeonOBJ, pack_factor) {
    pack_factor = pack_factor || 9;
    var midXY = get_cruciformCoord(dungeonOBJ);
 ...
 */
function get_cruciformCoord(dungeonOBJ) {
...
function emplace_room(dungeonOBJ, room_definition) {
    ...
    var banned_axis = dungeonOBJ['cruciformCoord'];
    ...
    while (new_room.blocked && counter > 0) {
        counter--;
        new_room = sound_room(dungeonOBJ, Math.max(top - 1, 1), Math.max(left - 1, 1), bottom + 1, right + 1);
    }
    // reject rooms that are blocked (sound.room determines blocked condition)
    if (new_room.blocked) return dungeonOBJ;

    var existing_room = Object.keys(new_room);
    var room_count = 0;
...
/**
 * Uses a directed graph to organize rooms and choose where to add corridors
 * @param {Dungeon} dungeonOBJ dungeon to add corridors to
 * @returns {Dungeon} DungeonOBJ with the corridors added
 */
...
function dig_graph_tunnels(dungeonOBJ) {
    ...
    for (i = 0; i < dungeonOBJ.graph_edges.length; i++) {
        //for (i = 0; i < 1; i++) {
        var room1_ID = dungeonOBJ.graph_edges[i].from + 1;
        var room2_ID = dungeonOBJ.graph_edges[i].to + 1;

        var r1 = dungeonOBJ.room.find(r => r.id === room1_ID);
        var r2 = dungeonOBJ.room.find(r => r.id === room2_ID);

        var room_from = Room.RoomFromOBJ(r1);
        var room_to = Room.RoomFromOBJ(r2);
...
/**
 * Dig a line from e,f to i,j opening cells as we go. Flag all as corridors.
 * @param {Dungeon} dungeonOBJ Dungeon to dig cells in
...
function stair_ends(dungeonOBJ) {
    ...
    for (d = 0; d < dungeonOBJ.n_i; d++) {
        ...
        for (g = 0; g < dungeonOBJ.n_j; g++) {
            var x = g * 2 + 1;
            if (dungeonOBJ.cell[y][x] == CORRIDOR) dungeonOBJ.cell[y][x] & STAIRS || $.each(stair_end,function (direction) {
...
function fix_doors(dungeonOBJ) {
    ...
    dungeonOBJ.room.each(function (room) {
        var id = room.id;
        $.each(room.door,function (direction) {
...
function get_palette(dungeonOBJ) {
    ...
    //     : palette.standard;
    var c;
    if ((c = palette_loc.colors) != null) $.each(c,function (d) {
        palette_loc[d] = c[d];
    });
...
function save_map() {
    save_canvas($('#map'), $('#dungeon_name').val() + ".png");
}

function do_print() {
    var userResolution = $('#Resolution').val();
