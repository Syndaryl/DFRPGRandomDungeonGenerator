...
function open_room(dungeon, room) {
    var c = door_sills(dungeon, room);
    if (!c.length) return dungeon;
    var d = alloc_opens(dungeon, room),
        e;
    for (e = 0; e < d; e++) {
        ...
        if (!(f & DOORSPACE))
            if (f = g.out_id) {
                f = [room.id, f].sort(cmp_int).join(",");
                if (!connect[f]) {
                    dungeon = open_door(dungeon, room, g);
                    connect[f] = 1
                }
            } else dungeon = open_door(dungeon, room, g)
    }
    return dungeon
}
...
function check_sill(a, room, x, y, wall) {
    var g = x + di[wall],
        f = y + dj[wall],
        h = a[g][f];
...
function open_door(dungeonOBJ, b, c) {
    ...
    if (e == ARCH) {
        dungeonOBJ.cell[doors][g] |= ARCH;
        f.key = "arch";
        f.type = "Archway"
    } else if (e == DOOR) {
        dungeonOBJ.cell[doors][g] |= DOOR;
        f.key = "open";
        f.type = "Unlocked Door"
    } else if (e == LOCKED) {
        dungeonOBJ.cell[doors][g] |= LOCKED;
        f.key = "lock";
        f.type = "Locked Door"
    } else if (e == TRAPPED) {
        dungeonOBJ.cell[doors][g] |=
            TRAPPED;
        f.key = "trap";
        f.type = "Trapped Door"
    } else if (e == SECRET) {
        dungeonOBJ.cell[doors][g] |= SECRET;
        f.key = "secret";
        f.type = "Secret Door"
    } else if (e == PORTC) {
        dungeonOBJ.cell[doors][g] |= PORTC;
        f.key = "portc";
        f.type = "Portcullis"
    }
    if (c) f.out_id = c;
