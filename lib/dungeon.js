...
function new_dungeon() {
    ...
    send_dungeon_details(dungeonOBJ);
    send_room_details(dungeonOBJ);
}

function send_dungeon_details(dungeonOBJ){
    var dungeonDiv = document.createElement('div');
    dungeonDiv.id = 'dungeon';
    dungeonDiv.className = 'dungeon';
    describe_light(dungeonDiv, dungeonOBJ['dungeon_overall']);
}

function send_room_details(dungeonOBJ) {
...
function describe_light(element, room){
    var contents = document.createElement('p');
    var lightDesc = room['light'].Description;
    if (room['light'].Penalty != 0) lightDesc +=  ' (' + room['light'].Penalty + ') light penalty';
...
function describe_rooms(dungeonOBJ) {
    ...
    for (roomIDX = 1; roomIDX <= dungeonOBJ.n_rooms; roomIDX++) {
        dungeonOBJ.room[roomIDX] = describe_room(dungeonOBJ.room[roomIDX]);
...
function light_dungeon(dungeonOBJ){
    ...
    dungeonOBJ.dungeon_overall.light.push(light.Description);
    return dungeonOBJ;
}
...
function tight_rooms(dungeon) {
    ...
    // currently, just allocating the specific size, and then packing in some rooms
    var b = alloc_rooms(dungeon,dungeon.room_size, 1),
        c;
    for (c = 0; c < b; c++) dungeon = emplace_room(dungeon);
    if (dungeon.huge_rooms) {
        b = alloc_rooms(dungeon, "tiny", 1);
        for (c = 0; c < b; c++) {
            var d = {
                size: "tiny"
            };
