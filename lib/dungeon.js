...
function html_dungeon_details(dungeonOBJ){
    ...
    html_travel_time(dungeon_key, dungeonOBJ.travel_distance);
...
function html_room_details(dungeonOBJ) {
    ...
    for (var roomItr = 1; roomItr <= dungeonOBJ.n_rooms; roomItr++) {
        ...
        room = treasure_room(room, dungeonOBJ.cer, div, dungeonOBJ.generosity, dungeonOBJ.travel_distance);
    }
}
...
function html_wandering_monsters(element, wandering_monsters, target_cer){
    ...
    list = list.sort((a,b) => parseInt(a.Weight) > parseInt(b.Weight));
    list = list.slice(0, Math.min(12,list.length));
    ...
    list.forEach(element => {
        var percent = Math.max(1, Math.floor( element.Weight / sum * 100 ));
        var this_target_cer = Math.round( target_cer * rand_between( 0.75, 2 ) );
        var quantity = Math.max( // at least one
            1, Math.round(this_target_cer / element.CER)
        );
        if (quantity > 4)
        {
        ...
        { quantity = '1d6-1 (min 1)'}
        else if (quantity == 2)
        { quantity = '1d6-2 (min 1)'}
        var row = document.createElement('tr');
        table.appendChild(row);
        if (Math.min(100,accumulate+percent) == accumulate)
            add_cell(row, Math.min(100,accumulate+percent));
        else
            add_cell(row, accumulate + '-' + Math.min(100,accumulate+percent));
        add_cell(row, quantity);
        add_cell(row, element.Description);
        add_cell(row, element.CER);
...
function open_door(dungeonOBJ, room, candidate) {
    ...
    door_obj.penalty = door_penalty(dungeonOBJ.cer);
...
/**
 * Randomly generates a modifier (bonus or penalty) to pick a door lock, then is adjusted by CER.
 * The base modifier is calcualted as (2d5 - 4) - yes, 2d5, not 2d6. This gives an average result clustering around "+1", but ranging anywhere from -3 to +6.
 * This modifier then has (CER / 75) subtracted from it. 
 * The overall skill modifier is capped to a maximum of +5 and a minimum of -6, and is rounded DOWN (-2.5 becomes -3 not -2).
 * @param {number} CER 
 */
function door_penalty (CER) {
    var roll = rand_between(1,5) + rand_between(1,5) -4;
    var cer_adjustment = (CER / 75);
    var min = -6;
    var max = 5;
    return Math.floor(Math.min( Math.max( roll - cer_adjustment, min), max))
}

/**
 * Randomly generates a modifier (bonus or penalty) to detect, disarm, sense or evade a trap, then is adjusted by CER.
 * The base modifier is calcualted as (2d7 - 6). This gives an average result clustering around "+2", but ranging anywhere from -4 to +8.
 * This modifier then has (CER / 66) subtracted from it. 
 * The overall skill modifier is capped to a maximum of +8 and a minimum of -8, and is rounded DOWN (-2.5 becomes -3 not -2).
 * @param {number} CER 
 */
function trap_penalty (CER){
    var roll = rand_between(1,7) + rand_between(1,7) - 6;
    var cer_adjustment = (CER / 66);
    var min = -8;
    var max = 8;
    return Math.floor(Math.min( Math.max( roll - cer_adjustment, min), max))
}
