 ...
 * @returns {Array} array of points to be turned into "junction nodes" for pathing
 */
function empty_points_from_space(dungeonOBJ, open_space, rooms) {
    console.debug('empty_points_from_space(open_space:' + open_space + ')');
    var points = [];
    open_space = Math.round(open_space / 2);
    var offset = Math.round(open_space / 2);
...
function node_from_id(dungeonOBJ, i) {
    ...
    i = i;
    var r1 = dungeonOBJ.room.find(r => r.id === i);
    if (r1) return Room.RoomFromOBJ(r1);

    var node = dungeonOBJ.junction_points[i - dungeonOBJ.n_rooms];
    console.debug('not a room! node_from_id #' + (i - 1 - dungeonOBJ.n_rooms) + ' is ' + JSON.stringify(node));
    node = node || [0, 0];
