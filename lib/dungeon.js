...
function init_form() {
    ...
    available_environments.each(function (env) {
        ...
        elem = new Element('input', { value: env, type: "checkbox", name: "biome_check" });
        var label = new Element('label', {for: 'env'}).update(labelText);
        $("biomes").append(label);
        $("biomes").append(hr);
        $("biomes_fields").append(elem);
    });
    ...
    $("biomes").observe("change", new_dungeon);
    $("biomes").observe("change", save_dungeon_configuration);
...
function save_dungeon_configuration(set_item_key) {
    ...
    dungeon_options_to_save["biomes"] = $("biomes").getValue();
