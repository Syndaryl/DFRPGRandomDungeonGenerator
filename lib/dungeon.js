...
function length_to_feet(width) {
    return width * 0.3;
}

function length_to_yards(width) {
    return length_to_feet(width) / 3;
}

function describe_description(element, room) {
    var description = document.createElement('p');
    add_labeled_text(description, 'Description: ',
        'This room is ' + length_to_feet(room['width'])
                     + ' feet wide and ' + length_to_feet(room['height'])
                     + ' feet long (' + length_to_yards(room['width'])
                     + 'x' + length_to_yards(room['height'])
                     + ' yards/hexes). ' +
...
function describe_monsters(element, room) {
    if (room['monsters'].length > 0) {
        ...
        var contents = document.createElement('p');
        var mons = [];
        var cer = monsters_cer(room['monsters']);
        
        mons.push("CER: " + cer + "; Threat: " + room['threat']);
 ...
 * @param {any} room
 */
function set_room(dungeon, room) {
    room.size || (room.size = dungeon.room_size);
    let room_from_opts = dungeon_options.room_size[room.size];
    let min_size = room_from_opts.size || 2;
    let radix = room_from_opts.radix || 5;
    
    if (!("height" in room))
        if ("i" in room) {
            let max_rand = dungeon.n_i - min_size - room.i;
            if (max_rand < 0) max_rand = 0;
            max_rand = min(max_rand, radix);
            room.height = random(max_rand) + min_size
        } else room.height = random(radix) + min_size;
    if (!("width" in room))
        if ("j" in room) {
            let max_rand = dungeon.n_j - min_size - room.j;
            if (max_rand < 0) max_rand = 0;
            max_rand = min(max_rand, radix);
            room.width = random(max_rand) + min_size
        } else room.width = random(radix) + min_size;
    "i" in room || (room.i = random(dungeon.n_i - room.height));
    "j" in room || (room.j = random(dungeon.n_j - room.width));
    return room
}
