...
function corridors_digraph(dungeonOBJ) {
    ...
    var edgecount_restore = Math.round(0.5 * sauce_edges.length);
    for (i = 0; i < edgecount_restore; i++) {
        if (sauce_edges.length === 0) break;
        var edge = sauce_edges.splice(random(sauce_edges.length), 1);
        if (edge != null) {
            console.debug('adding ' + JSON.stringify(edge) + ' to graph edges');
    ...
    for (i = 0; i < graph_edges.length; i++) {
        ...
        var room_from = Room.RoomFromOBJ( dungeonOBJ.room.find(r => r.id === (graph_edges[i].from+1)));
        var room_to = Room.RoomFromOBJ(dungeonOBJ.room.find(r => r.id === (graph_edges[i].to+1)));

        console.debug(' ROOM CENTERS ' + JSON.stringify({
            from: [
                room_from.id,
                room_from.center
            ],
        ...
        if (!(room_from.id === 0 || room_to.id === 0)) {
            delve_manhattan_tunnel(dungeonOBJ,
                room_from.center.h, room_from.center.v,
                room_to.center.h, room_to.center.v);
        } else {
 ...
 * @returns {Boolean} always true for some reason
 */
function delve_manhattan_tunnel(dungeonOBJ, e, f, i, j) {
    var x = [e, i].sort(cmp_int); // always go from left to right
    y = [f, j].sort(cmp_int); // always go from top to bottom
    var point1 = new Tuple(x[0], y[0]);
    var point2 = new Tuple(x[1], y[1]);
    var manhattan_midpoint1 = new Tuple(point1.X, point2.Y);
    var manhattan_midpoint2 = new Tuple(point2.X, point1.Y);

    if (random(100) < 50) {
        delve_tunnel(dungeonOBJ, point1.X, point1.Y, manhattan_midpoint1.X, manhattan_midpoint1.Y);
        delve_tunnel(dungeonOBJ, manhattan_midpoint1.X, manhattan_midpoint1.Y, point2.X, point2.Y);
    } else {
        delve_tunnel(dungeonOBJ, point1.X, point1.Y, manhattan_midpoint2.X, manhattan_midpoint2.Y);
        delve_tunnel(dungeonOBJ, manhattan_midpoint2.X, manhattan_midpoint2.Y, point2.X, point2.Y);
    }
    return true;
}
/**
 ...
 */
function base_layer(image, mapdetails, base_context) {
    ...
    var context = canvas.getContext("2d"),
        ...
        bgColor;
    bgColor = palette.open || palette.white;
    ...
    for (i = 0; i <= dungeonOBJ.n_rows; i++) {
        ...
        for (j = 0; j <= dungeonOBJ.n_cols; j++) {
            ...
            if (dungeonOBJ.cell[i][j] & CORRIDOR) {
                dungeon_image.drawImage(canvas, x, y, rez, rez, x, y, rez, rez);
            }
