...
//import * as archesList from '../data/archesList.json';
jQuery.noConflict();

//var archesList;
//var data_folder = window.location.origin + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/")) + '/data/';
//var json_url = data_folder + 'archesList.json';
...
function html_dungeon_details(dungeonOBJ){
    ...
    html_donjonTSV(dungeonOBJ, dungeon_debug, dungeonOBJ.cell);
}

/**
 * dumps out the dungeon in a TSV format suitable for import into Dungeon Painter Studio, similar to the format from Donjon
 * @param {Dungeon} dungeonOBJ - dungeon data source
 * @param {HTMLDivElement} element - to display the TSV data in
 * @param {Array<Array<number>>} cell - array of dungeon cells
 * @returns {null} doesn't return anything.
 */
function html_donjonTSV(dungeonOBJ, element, cell) {
    for (var i = 0; i < cell.length; i++) {
        var row = new Array;
        for (var j = 0; j < cell[i].length; j++) {
            var door_key = '[' + i + ',' + j + ']';
            row[j] = '';
            if (cell[i][j] & STAIR_DN) {
                row[j] += 'SD';

            } else if (cell[i][j] & STAIR_UP) {
                row[j] += 'SU';

            }else if (cell[i][j] & OPENSPACE) {
                row[j] += 'F';
            }
            else if (cell[i][j] & DOORTABLE) {
                row[j] += 'D';
                if (cell[i][j] & TRAPPED) {
                    row[j] += 'T';
                }
 ...
 * 
 * @param {HTMLDivElement} element - to display the TSV data in
 * @param {Array} data to be dumped
 * @returns {null} writes directly to element
 */
function tsv_debug_dump(element, data) {
...
function html_room_doors(element, room) {
    ...
    for(var d = 0; d < 4; d++){
        ...
        for (var i = 0; i < door_count; i++)
        {
            ...
            door_name = title_case_sentance(dir+ ' ' + 'door')  + (door_count > 1 ?' ' + ( String.fromCharCode('a'.charCodeAt(0) + i) ):'') + ': ';
...
function travel_distance_dungeon(dungeonOBJ) {
    ...
    {
        console.debug("Random distance!");
        travelDistance = select_from_weighted_table(travel_times).Type;
        console.debug("Random distance set to " + JSON.stringify(travelDistance));
    }
    else
    {
        console.debug("Static distance " + dungeonOBJ.travel_distance + "!");
        travelDistance = dungeonOBJ.travel_distance;
        console.debug("Static distance set to " + JSON.stringify(travelDistance));
    }
    console.debug("travel distance is using this object " + JSON.stringify(dungeon_options['travel_distance'][dungeonOBJ.travel_distance]));
    ...
    console.debug("dungeonOBJ.dungeon_overall.travel_distance is using this object " + JSON.stringify(dungeonOBJ.dungeon_overall.travel_distance));

    console.debug("Travel Distance processed in travel_distance_dungeon() as " + JSON.stringify(travelDistance));
    return dungeonOBJ;
}
 ...
 * This is to make the dungeon feel more "sensible", and also to add variety of hallway widths before my players riot about the tightness.
 * @param {Dungeon} dungeonOBJ
 * @returns {null} no return - modifies the dungeon directly
 */
function experimental_rooms(dungeonOBJ) {
    dungeonOBJ = main_hallways(dungeonOBJ);
    //var room_count = Math.max(1,  Math.round( alloc_rooms(dungeonOBJ, null, 9) ));
    //dungeonOBJ.n_rooms = room_count;
    //for (c = 0; c < room_count; c++)
    //    dungeonOBJ = emplace_room(dungeonOBJ, dungeonOBJ.room_size);
    return tight_rooms(dungeonOBJ, 11);
...
function main_hallways(dungeonOBJ) {
    ...
    delve_tunnel(dungeonOBJ, midX, 0, midX, dungeonOBJ.max_row);
    delve_tunnel(dungeonOBJ, 0, midY, dungeonOBJ.max_col, midY);
...
function tight_rooms(dungeonOBJ, pack_factor) {
    ...
    for (var roomSizeCurrent = roomMax; roomSizeCurrent >= 0; roomSizeCurrent--){
        roomCount = alloc_rooms(dungeonOBJ, dungeon_options.room_size[index[roomSizeCurrent]], pack_factor);
        roomCount = Math.max(1, roomCount - 2);
...
/**
 * Places rooms in a loose pattern with room for hallways.
 * @param {Dungeon} dungeonOBJ The dungeon to place rooms in
 * @param {number} pack_factor the "packing" factor. The higher the pack_factor, the more "empty space"
 * @returns {null} no return - modifies the dungeon directly
 */
function scatter_rooms(dungeonOBJ, pack_factor) {
    pack_factor = pack_factor || 9;
    var room_count = alloc_rooms(dungeonOBJ, null, pack_factor),
        c;
    for (c = 0; c < room_count; c++) dungeonOBJ = emplace_room(dungeonOBJ);
    if (dungeonOBJ.huge_rooms) { // alocate some medium rooms to squeeze in amongst the huge ones
        room_count = alloc_rooms(dungeonOBJ, "medium");
        for (c = 0; c < room_count; c++) {
            var d = {
                size: "medium"
            };
 ...
 * @returns {null} no return - modifies the dungeon directly
 */
function sparse_rooms(dungeonOBJ, pack_factor) {
    pack_factor = pack_factor || 15;
    var b = alloc_rooms(dungeonOBJ, null, pack_factor),
        c;
...
/**
 * Place rooms in the size specified and then spot filling every size below so the dungion 
 * is quite packed, but still has room for hallways between many rooms.
 * @param {Dungeon} dungeonOBJ
 * @returns {null} no return - modifies the dungeon directly
 */
...
/**
 * Takes the dungeon provided and figures out how many rooms should be allocated to it, 
 * given the room size and desired packing.
 * @param {Dungeon} dungeonOBJ Dungeon to check for room space.
 * @param {String} room_size_name If provided, use only this room size. If null, use the room size for the dungeon.
 * @param {Number} packing used to determine the amount of "personal space" rooms should have - higher = more empty space
 * @returns {null} nothing - modifies dungeonOBJ directly.
 */
...
function emplace_room(dungeonOBJ, room_definition) {
    ...
    var room = room_definition || new Room();
    ...
    // reject rooms that leave the dungeon or are blocked
    if (top < 1 || bottom > dungeonOBJ.max_row) return dungeonOBJ;
    if (left < 1 || right > dungeonOBJ.max_col) return dungeonOBJ;
    var existing_room = sound_room(dungeonOBJ, top, left, bottom, right);
    if (existing_room.blocked) return dungeonOBJ;

    existing_room = $H(existing_room).keys();
    var room_count = 0;
    if (existing_room.length == 0) {
        room_count = dungeonOBJ.n_rooms + 1;
        dungeonOBJ.n_rooms = room_count;
    } else if (existing_room.length == 1)
 ...
 * @param {Dungeon} dungeonOBJ
 * @param {Room} room
 * @returns {Room} room with its size and coordinates set
 */
function set_room(dungeonOBJ, room) {
    room.size || (room.size = dungeonOBJ.room_size);
    var roomSizeDef = dungeon_options.room_size[room.size],
        room_size = 0;
    room_size = roomSizeDef.size || 2;
    var radix = roomSizeDef.radix || 5;
    var size_factor = 0;
    if (room.height === undefined || room.height < 1)
        if (room.i === undefined) {
            size_factor = dungeonOBJ.n_i - room_size - room.i;
            if (size_factor < 0) size_factor = 0;
            size_factor = size_factor < radix ? size_factor : radix;
            room.height = random(size_factor) + room_size;
        } else room.height = random(radix) + room_size;
    if (room.width === undefined || room.width < 1)
        if (room.j === undefined) {
            size_factor = dungeonOBJ.n_j - room_size - room.j;
            if (size_factor < 0) size_factor = 0;
            size_factor = size_factor < radix ? size_factor : radix;
            room.width = random(size_factor) + room_size;
        } else room.width = random(radix) + room_size;
    room.i = random(dungeonOBJ.n_i - room.height);
    room.j = random(dungeonOBJ.n_j - room.width);
    return room;
}
...
function open_rooms(dungeonOBJ) {
    ...
    connect = {};
    if (dungeonOBJ.n_rooms === undefined) {
        console.error("No number of rooms in open_rooms()!!!");
        console.trace();
        return dungeonOBJ;
    }
    var room;
 ...
 * TODO: define "excess doors" - only one door per wall? 
...
function corridors_connector(dungeonOBJ){
    ...
    while (unconnected_rooms.length > 0) {
        ...
        var orphan_doors = shuffle(orphan.door[dir]);
...
/**
 * Starting at [i,j] dig a one wide tunnel aimlessly.
 * @param {Dungeon} dungeonOBJ Dungeon to place tunnel in
 * @param {Number} i starting X
 * @param {Number} j starting Y
 * @param {string} in_dir  direction we're headed in at the moment
 * @returns {Dungeon} the original dungeon, now modified
 */
function tunnel(dungeonOBJ, i, j, in_dir) {
    var valid_dirs = tunnel_dirs(dungeonOBJ.straight_pct, in_dir);
    for (var d = 0; d < valid_dirs.length; d++) {
        var dir = valid_dirs[d];
        var valid = open_tunnel(dungeonOBJ, i, j, dir);
        if (valid) {
            var x = i + di[dir],
                y = j + dj[dir];
            dungeonOBJ = tunnel(dungeonOBJ, x, y, dir); // recurse madly off in all directions
        }//else return dungeonOBJ;
    };
    return dungeonOBJ;
}
...
/**
 * Open a tunnel of significant size in the indicated direction if possible. 
 * Tunnel segment continues to edge of map. Return success or failure.
 * @param {Dungeon} dungeonOBJ Dungeon to place tunnel in
 * @param {Number} width width of hallway - must be an odd number. Defaults to 3.
 * @param {Number} i starting X
 * @param {Number} j starting Y
 * @param {String} direction the cardinal direction we are going in from this location
 * @param {bool} relentless if true barge through other tunnels to a room or edge of map
 * @returns {bool} indicating if the tunnel segment was opened
 */
function large_tunnel(dungeonOBJ, width, i, j, direction, relentless) {
    width = width || 3;
    relentless = relentless || true;
    if (direction === 'east' || direction === 'west')
        delve_tunnel(dungeonOBJ, i, j, dungeonOBJ.width, j);
    else 
        delve_tunnel(dungeonOBJ, i, j, i, dungeonOBJ.height);
}

function large_tunnel_segment(dungeonOBJ, i, j, direction, relentless) {
    if (open_tunnel(dungeonOBJ, i, j, direction)) {
        var x = i + di[direction],
            y = j + dj[direction];
        return large_tunnel_segment(dungeonOBJ, x, y, direction, relentless);
 ...
 * @param {Number} straight_pct likelyhood to go straight
 * @param {string} in_dir  direction we're headed in at the moment
 * @returns {String[]} a suffled list of the directions.
 */
function tunnel_dirs(straight_pct, in_dir) {
    var shuffledDirections = shuffle(dj_dirs);
    if (in_dir !== null && straight_pct !== null) {
        if (random(100) < straight_pct)
            shuffledDirections.unshift(in_dir);
    }
 ...
 * @param {Array} deck array of stuff that needs to be shuffled.
 * @returns {Array} a new copied array with the contents shuffled into a different order.
 */
 ...
 * @returns {bool} indicating if the tunnel segment can be opened
 */
 ...
 * @returns {bool} indicating if the tunnel segment was opened
 */
function delve_tunnel(dungeonOBJ, x1, y1, x2, y2) {
    var x = [x1, x2].sort(cmp_int); // always go from left to right
        y = [y1, y2].sort(cmp_int); // always go from top to bottom
    for (y2 = x[0]; y2 <= x[1]; y2++)
        for (x2 = y[0]; x2 <= y[1]; x2++) {
            dungeonOBJ.cell[y2][x2] &= ~ENTRANCE;
            dungeonOBJ.cell[y2][x2] |= CORRIDOR;
        }
    return true;
}
...
/**
 * Places some stairs.
 * @param {Dungeon} dungeonOBJ Dungeon to place stairs in
 * @returns {Dungeon} the dungeon modified
 */
...
function base_layer(a, b) {
    ...
    var d = c.getContext("2d"),
        ...
        h;
    h = palette.open;
    h != null ? fill_rect(d, 0, 0, e, g, h) : fill_rect(d, 0, 0, e, g, palette.white);
    if ((h = palette.open_grid) != null) image_grid(a, b, h, d);
    else if ((h = palette.grid) != null) image_grid(a, b, h, d);
    return c;
}
...
function image_doors(dungeonOBJ, map_details, canvas) {
    ...
    for (var i = 0; i < rooms.length; i++) {
        if (rooms[i] != undefined) {
            var doors = new Doors(rooms[i].door["north"], rooms[i].door["south"], rooms[i].door["east"], rooms[i].door["west"]);
            for (var j = 0; j < 4; j++) {
                if (doors[dj_dirs[j]] != [] && doors[dj_dirs[j]] != undefined) {
                    var len = doors[dj_dirs[j]].length;
                    for (var k = 0; k < len; k++) {
                        door_object = doors[dj_dirs[j]][k];
                        image_text(door_object.sill_r, door_object.sill_c, canvas, String.fromCharCode( 'a'.charCodeAt(0) + k), map_details);
