            ...
            if (dungeonOBJ.cell[x][y] & OPENSPACE) {
                ...
                if (color = pal.bevel_nw) {
                    dungeonOBJ.cell[x][y - 1] & OPENSPACE || 
                        draw_line(dungeon_image, y_offset - 1, x_offset, y_offset - 1, x_offset_p, color);
                    dungeonOBJ.cell[x - 1][y] & OPENSPACE || 
                        draw_line(dungeon_image, y_offset, x_offset - 1, y_offset_p, x_offset - 1, color);
                    if (color = pal.bevel_se) {
                        dungeonOBJ.cell[x][y + 1] & OPENSPACE || 
                            draw_line(dungeon_image, y_offset_p + 1, x_offset + 1, y_offset_p + 1, x_offset_p, color);
                        dungeonOBJ.cell[x + 1][y] & OPENSPACE || 
                            draw_line(dungeon_image, y_offset + 1, x_offset_p + 1, y_offset_p, x_offset_p + 1, color);
                    }
                } else if (color = pal.wall_shading) {
                    dungeonOBJ.cell[x -
                        1][y - 1] & OPENSPACE || wall_shading(dungeon_image, y_offset - rez_4th, x_offset - rez_4th, y_offset - 1, x_offset - 1, color);
