...
function html_dungeon_details(dungeonOBJ){
    ...
    html_wandering_monsters(dungeonDiv, dungeonOBJ['wandering_monsters'], parseInt(dungeonOBJ.cer));
...
function html_wandering_monsters(element, wandering_monsters, target_cer){
    if (wandering_monsters.length == 0)
        return;
    var sum = wandering_monsters.reduce(function ( previous, current){
    ...
    list.forEach(element => {
        var percent = Math.round( element.Weight / sum * 100 );
        var this_target_cer = target_cer * rand_between( 0.75, 2 );
        ...
        if (quantity > 4)
        {
            ...
            var remain = Math.round(quantity % 3.5);
            quantity = dice + 'd6 ' + (
             remain > 0? '+' + remain:
             (remain < 0? remain :
             ''));
