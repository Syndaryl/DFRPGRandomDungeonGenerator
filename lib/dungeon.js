...
function door_sills(dungeon, room) {
    console.log(JSON.stringify({"dungeon cell": dungeon.cell, "room ID": room.id, "complex?": room.complex}));
 ...
 */
    ...
    console.log(JSON.stringify({x, y, wall, target_cell, out_cell}));
    if (!(out_cell & PERIMETER)) return false;
    if (out_cell & DOOR) return false;
    var h = out_X + di[wall];
    var i = out_Y + dj[wall];
    if (target_cell & BLOCKED) return false;
    var out_ID = (cells & ROOM_ID) >> 6;
    if (cells == room.id) return false;
    return {
        sill_r: x,     // sill_r: 
        sill_c: y,     // sill_c: 
        dir:    wall,  // dir:    
