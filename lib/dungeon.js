...
function emplace_room(dungeonOBJ, room_definition) {
    ...
    for (var h = top; h <= bottom; h++) {
        ...
        for (i = left; i <= right; i++) {
            ...
            dungeonOBJ.cell[h][i] |= ROOM | room_count << ROOM_SHIFT;
        }
    }
    ...
    var existing_room = dungeonOBJ.room[room_count];
    ...
    for (top = top; top <= bottom; top++) {
        ...
        for (f = left; f <= right; f++) {
            ...
            if (dungeonOBJ.cell[top][f] & ROOM) {
                var h = (dungeonOBJ.cell[top][f] & ROOM_ID) >> ROOM_SHIFT; // this has to do with complex rooms in some way
...
function prune_doors(dungeonOBJ) {
    ...
    // dungeonOBJ.room.forEach( room => {
    //     master_doors.push({"room":room.id, "doors":room.door, "complex": dungeonOBJ.complex_rooms});
    // });
    var doors = [];
 ...
 */
    ...
    var out_ID = (out_cell & ROOM_ID) >> ROOM_SHIFT;
...
function label_rooms(dungeonOBJ) {
    ...
    for (roomIDX = 1; roomIDX <= dungeonOBJ.n_rooms; roomIDX++) {
        ...
        var f;
        for (f = 0; f < e; f++) dungeonOBJ.cell[g][rm + f] |= d.charCodeAt(f) << 24; // not related to other 24s!
    }
    return dungeonOBJ;
}
