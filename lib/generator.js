...
function select_from_weighted_table(table) {
    if (table.constructor != Array) {
        console.trace("table is not an Array");
        return {};
    }
    if (!table[0].hasOwnProperty('Weight')) {
        console.trace("table is not an Array of stuff with Weights");
        return {};
    }

    var swept_table = table.filter(function (entry) {
        return entry.Weight > 0;
    });

    var sumOfWeights = swept_table.reduce(function (memo, entry) { return memo + entry.Weight; }, 0);
    var selectedWeights = {};
    ...
    console.trace("table ran out of entries !?");
